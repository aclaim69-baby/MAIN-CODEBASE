-- ================================================================
--  DAILY CHECKING APP — LAST UPDATED TRACKING + FULL SYNC SETUP
...
-- ================================================================
-- SECTION 1 — RECORDS TABLE (exact columns from sync.ts)
-- ================================================================
...
-- Remove duplicate records (keep newest per ref_id)
DELETE FROM records
WHERE id IN (
  SELECT id FROM (
    SELECT id,
...
-- Performance index on submitted_at (matches ORDER BY in fetchRemoteRecords)
...
-- Stored BIGINT column for dedup index (avoids immutable function error)
...
-- Remove near-duplicate log entries (same content within 5-second window)
DELETE FROM activity_logs
WHERE ctid NOT IN (
  SELECT MIN(ctid)
  FROM activity_logs
...
-- Safe dedup index using the stored computed column
...
-- ================================================================
-- SECTION 3 — LAST UPDATED TRACKING TABLE
-- Stores the timestamp of the most recent change to any table.
-- The frontend polls this to display "Last updated: Xs ago".
-- ================================================================
...
-- ================================================================
-- SECTION 4 — FUNCTION: stamp sync_status on any table change
-- Called by triggers on records and activity_logs.
-- ================================================================

CREATE OR REPLACE FUNCTION fn_stamp_last_updated()
RETURNS TRIGGER AS $$
BEGIN
  UPDATE sync_status
...
-- ================================================================
-- SECTION 5 — TRIGGERS: stamp last_updated on records changes
-- ================================================================

DROP TRIGGER IF EXISTS trg_stamp_records_insert ON records;
CREATE TRIGGER trg_stamp_records_insert
  AFTER INSERT ON records
  FOR EACH ROW EXECUTE FUNCTION fn_stamp_last_updated();

DROP TRIGGER IF EXISTS trg_stamp_records_update ON records;
CREATE TRIGGER trg_stamp_records_update
  AFTER UPDATE ON records
  FOR EACH ROW EXECUTE FUNCTION fn_stamp_last_updated();

DROP TRIGGER IF EXISTS trg_stamp_records_delete ON records;
CREATE TRIGGER trg_stamp_records_delete
  AFTER DELETE ON records
  FOR EACH ROW EXECUTE FUNCTION fn_stamp_last_updated();


-- ================================================================
-- SECTION 6 — TRIGGERS: stamp last_updated on activity_logs changes
-- ================================================================

DROP TRIGGER IF EXISTS trg_stamp_logs_insert ON activity_logs;
CREATE TRIGGER trg_stamp_logs_insert
  AFTER INSERT ON activity_logs
  FOR EACH ROW EXECUTE FUNCTION fn_stamp_last_updated();

DROP TRIGGER IF EXISTS trg_stamp_logs_delete ON activity_logs;
CREATE TRIGGER trg_stamp_logs_delete
  AFTER DELETE ON activity_logs
  FOR EACH ROW EXECUTE FUNCTION fn_stamp_last_updated();


-- ================================================================
-- SECTION 7 — AUTO LOG TRIGGER: insert activity_log on record submit
-- ================================================================
...
-- ================================================================
-- SECTION 8 — AUTO LOG TRIGGER: insert activity_log on record delete
-- ================================================================
...
-- Fix super admin row — delete and reinsert clean
...
-- Remove rogue data column and rescue logo values
DO $$
BEGIN
...
-- ================================================================
-- SECTION 12 — DISABLE ROW LEVEL SECURITY ON ALL TABLES
-- CRITICAL: RLS with no policies silently returns 0 rows
-- ================================================================
...
-- ================================================================
-- SECTION 13 — REPLICA IDENTITY FULL
...
-- ================================================================
-- SECTION 14 — ADD TABLES TO REALTIME PUBLICATION
-- Without this, WebSocket shows SUBSCRIBED but fires zero events
-- ================================================================
...
-- Inspect every result after running. All must pass.
-- ================================================================
...
-- 15B: Must show 3 rows — records, activity_logs, sync_status in publication
...
-- 15C: Must show relreplident = 'f' (FULL) for all 3 realtime tables
SELECT relname, relreplident
FROM pg_class
WHERE relname IN ('records', 'activity_logs', 'sync_status')
ORDER BY relname;

-- 15D: Must show 5 triggers (insert+update+delete on records, insert+delete on logs)
SELECT trigger_name, event_manipulation, event_object_table
FROM information_schema.triggers
WHERE trigger_name IN (
  'trg_log_record_insert',
  'trg_log_record_delete',
  'trg_stamp_records_insert',
  'trg_stamp_records_update',
  'trg_stamp_records_delete',
  'trg_stamp_logs_insert',
  'trg_stamp_logs_delete'
)
ORDER BY event_object_table, trigger_name;

-- 15E: RLS must be false for all 9 tables
SELECT relname, relrowsecurity AS rls_enabled
FROM pg_class
WHERE relname IN (
  'records','activity_logs','admins','settings','sync_status',
  'checklist_templates','departments','sections','equipment_types'
)
ORDER BY relname;

-- 15F: Settings must have exactly 1 row with id = 1
SELECT id, footer_text, qc_required FROM settings;

-- 15G: sync_status must have exactly 1 row
SELECT id, last_updated, updated_table FROM sync_status;

-- 15H: Must return 0 rows — no duplicate ref_id in records
...
-- 15I: Dedup index must exist on activity_logs
SELECT indexname FROM pg_indexes
WHERE tablename = 'activity_logs'
  AND indexname = 'activity_logs_dedup_safe';

-- 15J: checklist column must be JSONB not TEXT
SELECT column_name, data_type
FROM information_schema.columns
WHERE table_name = 'records'
  AND column_name = 'checklist';

-- 15K: No rogue columns in activity_logs
SELECT column_name
FROM information_schema.columns
...
-- ================================================================
-- DONE. Check 15A through 15K above — all must show expected values.
-- ================================================================
