...
-- Remove duplicate records keeping the most recent per ref_id
DELETE FROM records
WHERE id IN (
  SELECT id FROM (
    SELECT id,
...
-- Add unique constraint on ref_id
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Performance index on submitted_at DESC
...
-- Add created_at_sec as a stored BIGINT for safe dedup indexing
-- (avoids the immutable function error from using date_trunc in an index)
...
-- Remove near-duplicate logs (same content within 5 seconds)
DELETE FROM activity_logs
WHERE ctid NOT IN (
  SELECT MIN(ctid)
  FROM activity_logs
...
-- Create safe dedup index using stored column (no immutable error)
...
-- DELETE and REINSERT super admin with correct values
...
-- Detect if id column is UUID type and fix it
DO $$
DECLARE
  col_type TEXT;
  saved_footer TEXT;
...
BEGIN
  ...
  IF col_type = 'uuid' OR col_type = 'character varying' THEN
    -- Save existing data
    BEGIN
    ...
    EXCEPTION WHEN OTHERS THEN
      ...
      saved_qc := FALSE;
    END;
    -- Drop and recreate with correct INTEGER id
...
-- Create settings table if it doesn't exist at all
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='settings' AND column_name='data') THEN
    -- Rescue logo data before dropping
    UPDATE settings s
...
-- Remove any rows that are not id=1
DELETE FROM settings WHERE id <> 1;

-- Ensure the single canonical row exists
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'settings_id_check'
  ) THEN
...
-- Fix items column type if wrong
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
...
-- Remove duplicate templates keeping latest
...
-- ============================================================
-- SECTION 9 — DISABLE ROW LEVEL SECURITY ON ALL TABLES
-- This is CRITICAL — RLS with no policies returns 0 rows
-- silently to the anon/publishable key. No error is shown.
-- ============================================================
...
-- ============================================================
-- SECTION 10 — REPLICA IDENTITY FULL
-- Required so DELETE realtime events carry payload.old.id
-- Without FULL, payload.old = {} and deletions don't sync
-- ============================================================
...
-- ============================================================
-- SECTION 11 — ADD TABLES TO REALTIME PUBLICATION
-- Without this, WebSocket shows SUBSCRIBED but fires ZERO events
-- This is the #1 reason updates don't appear on other devices
-- ============================================================
...
-- ============================================================
-- SECTION 12 — AUTO LOG TRIGGER ON RECORD INSERT
-- Fires AFTER INSERT on records
-- Inserts into activity_logs automatically
-- 5-second dedup guard prevents double logging
-- EXCEPTION block ensures trigger NEVER blocks record insert
-- ============================================================
...
-- ============================================================
-- SECTION 13 — AUTO LOG TRIGGER ON RECORD DELETE
-- Fires AFTER DELETE on records
-- ============================================================
...
-- Run these after the script to confirm every fix is in place
-- ============================================================
...
-- 14B: Must show 2 rows — records and activity_logs in realtime publication
SELECT pubname, tablename
FROM pg_publication_tables
WHERE pubname = 'supabase_realtime'
  AND tablename IN ('records', 'activity_logs');

-- 14C: Must show relreplident = 'f' (FULL) for both tables
SELECT relname, relreplident
FROM pg_class
WHERE relname IN ('records', 'activity_logs');

-- 14D: Must show 2 triggers — insert and delete
SELECT trigger_name, event_manipulation, event_object_table
FROM information_schema.triggers
WHERE trigger_name IN ('trg_log_record_insert', 'trg_log_record_delete');

-- 14E: Must show rowsecurity = false for all 8 tables
...
-- 14G: No duplicate ref_id values in records
...
-- 14H: activity_logs dedup index must exist
SELECT indexname FROM pg_indexes
WHERE tablename = 'activity_logs'
  AND indexname = 'activity_logs_dedup_safe';

-- 14I: checklist column must be JSONB in records
SELECT column_name, data_type
FROM information_schema.columns
WHERE table_name = 'records'
  AND column_name = 'checklist';

-- 14J: submitted_at column must exist in records
SELECT column_name, data_type
FROM information_schema.columns
WHERE table_name = 'records'
  AND column_name = 'submitted_at';

-- 14K: activity_logs must NOT have rogue columns
SELECT column_name
FROM information_schema.columns
WHERE table_name = 'activity_logs'
  AND column_name IN ('event_type', 'color', 'data', 'payload');
-- Above must return 0 rows

-- ============================================================
-- DONE — All fixes applied. Check results of 14A through 14K.
-- ============================================================
